    ...
    <>
      <header className="border-b border-border bg-surface px-6 py-4 flex justify-between items-center sticky top-0 z-10">
        ...
        <nav className="flex gap-4">
          <a href="https://github.com/khalil-sohail/chess-engine" target="_blank" rel="noreferrer" className="text-sm font-medium text-muted hover:text-foreground transition-colors">
            GitHub
          </a>
          <button onClick={() => setIsArchOpen(true)} className="text-sm font-medium text-muted hover:text-foreground transition-colors">
            Architecture
          </button>
        </nav>
      </header>
      ...
      {isArchOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-surface-elevated border border-border w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-4 border-b border-border">
              <h2 className="text-lg font-bold font-mono text-foreground">Architecture</h2>
              <button onClick={() => setIsArchOpen(false)} className="text-muted hover:text-foreground transition-colors">
