...
assert.match(panel, /cancelAnimationFrame/);
...
const backdropZIndex = Number(styles.match(/\.backdrop \{[\s\S]*?z-index: (\d+);/)?.[1]);
const mobilePanelZIndex = Number(styles.match(/\.panel\[data-mobile-overlay="true"\] \{[\s\S]*?z-index: (\d+);/)?.[1]);
const setupStyles = read('src/components/setup/SessionSetup.module.css');
const setupZIndex = Number(setupStyles.match(/\.backdrop \{[\s\S]*?z-index: (\d+);/)?.[1]);
assert.ok(backdropZIndex < mobilePanelZIndex, 'mobile panel must render above its backdrop');
assert.ok(mobilePanelZIndex < setupZIndex, 'setup dialog must render above the responsive panel');
